...
export const Trello = () => {
    ...
    // перетаскивание между картами И внутри одной карты
    ...
    const handleTaskDragOver = (e: any, targetCardId: number) => {
        e.preventDefault();
        e.stopPropagation();
    };
    ...
    const handleTaskDropSameCard = (e: any, targetTaskId?: number) => {
        e.preventDefault();
        e.stopPropagation();
        
        if (draggedTask) {
            handleTaskDrop(e, draggedTask.sourceCardId, targetTaskId);
        } else {
            setDraggedTask(null);
        }
    };
    ...
    return (
        <div className="trello_board" onClick={handleClickOutside}>
            <div className="trello_cards">
                {card.map((cardItem) => (
                    <div key={cardItem.id} className="card" 
                        draggable
                        ...
                        <div className="card_header d-flex justify-content-between align-items-center flex-row">
                            <div className="card_title d-flex flex-row">
                                ...
                                ) : (<h1 onClick={() => handleCardTitleClick(cardItem)}>{cardItem.title}</h1>)}
                            </div>
                                ...
                                <div className="tasks-list">
                                    {cardItem.tasks.map((task) => (
                                        <div key={task.id} className="task-item" 
                                            ...
                                            onDrop={(e) => handleTaskDropSameCard(e, task.id)}> 
                        ...
                        <div className="add-task-footer d-flex " onClick={(e) => {
                                    e.stopPropagation();
                                    addTask(cardItem.id);}}>
                            <button
